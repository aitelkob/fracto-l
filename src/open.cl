...
int  get_color(int   maxitration,int n, double  red,double  blue,double  green)
{
    double t;
    int     r,g,b;

    t = (double)n / (double)maxitration;
    r = (int)(9*(1-t)*t*t*t*255)+ red;
    g = (int)(15*(1-t)*(1-t)*t*t*255) + blue;
    b = (int)(8.5*(1-t)*(1-t)*(1-t)*t*255) + green;
...
{	
		...
		int	shape 		= buffer[7];
		double	red 	= buffer[8];
		double	blue 	= buffer[9];
		double	green 	= buffer[10];
		int	color = 0;
		n = 0;
		res = 0;
	    ...
	    if (shape == 4)
		 ...
		 if (shape == 5)
          ...
          if (shape == 6)
          {
			...
			color  = get_color(maxitration,n,red,blue,green);
		if (n == maxitration)
		{
				ouput[idx] = 0;
				ouput[idx+1] = 0;
