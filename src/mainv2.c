...
/*   Updated: 2019/12/23 20:34:18 by yait-el-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		setup_cl(t_mlix *mlix)
{
	...
	ret = 0;
	mlix->opencl.constants = (double *)malloc(sizeof(double) * 11);
	...
	mlix->opencl.buffer = clCreateBuffer(mlix->opencl.context,
			CL_MEM_READ_WRITE, sizeof(double) * 11, NULL, &ret);
...
void		open_help(t_mlix *mlix)
{
	mlix->ret = clEnqueueWriteBuffer(mlix->opencl.command_queue,
			...
			sizeof(cl_double) * 11, mlix->opencl.constants, 0, NULL, NULL);
